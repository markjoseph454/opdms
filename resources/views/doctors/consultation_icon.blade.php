<div class="row">
    <div class="col-md-7" style="display:inline">
        ...
        <button class="btn btn-default menusConsultations" onclick="medicalRecords({{ $patient->id }})" title="View medical record's">
            <i class="fa fa-file-text-o text-success"></i>
        </button>
        ...
        @if(Auth::user()->clinic == 8)
            <button class="btn @if($smoke) btn-danger @else btn-default @endif menusConsultations" data-status="@if($smoke) on @else off @endif" title="Smoke Cessation"
    ...
    <div class="col-md-5 text-right icd10codes" style="display:inline">
        ...
        @if(Session::has('cid'))
            <a href="{{ url('createAnewForm') }}" class="btn btn-default menusConsultations"
            ...
            <a href="{{ url('print/'.Session::get('cid')) }}" class="btn btn-default menusConsultations"
