...
function forpending(pending){
	...
	if (pending.length > 0) {
		for (var i = 0; i < pending.length; i++) {
			var tr = $('<tr>');
			if (pending[i].cashid) {
				if (pending[i].mss_id >= 9 && pending[i].mss_id <= 13 || pending[i].price <= 0) {
			...
			if (pending[i].cashid) {
				if (pending[i].mss_id >= 9 && pending[i].mss_id <= 13 || pending[i].price <= 0) {
					var td9 = '<td align="center" class="info" id="rstatus">FREE</a></td>';
				}
				else{
					var td9 = '<td align="center" class="info" id="rstatus">PAID</a></td>';
				}
			}else{
				var td9 = '<td align="center" class="info" id="rstatus">PENDING</a></td>';
			}
			var td10 = $('<td align="center">').text(pending[i].created_at);
			var tr2 = $('</tr>');
			$(tr).append(td1,td2,td3,td4,td5,td6,td7,td10,td9,td8,tr2);
			if (pending[i].get != "Y") {
		...
		qty: qty,
			...
			function (data, status){
				...
				if (request.cashincome == null) {
					...
					$(scope).closest('tr').find('#rstatus').text("FREE");
		...
		function (data, status){
			...
			if (request.length > 0) {
				for (var i = 0; i < request.length; i++) {
					var tr = $('<tr>');
					var td9 = $('<td align="center">').text(request[i].or_no);
					...
					total_amount += amount;
					var td4 = $('<td align="right" hidden>').text(amount.toFixed(2));
					var td5 = $('<td align="right" hidden>').text(request[i].discount.toFixed(2));
					...
					var td8 = $('<td>').text(request[i].dates);
					var tr2 = $('</tr>');
					$(tr).append(td9, td1, td2, td3, td4, td5, td6, td7, td8, tr2);
